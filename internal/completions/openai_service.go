...
var _ AICompletioner = (*OpenAIService)(nil)

func NewOpenAIService(config *OpenAIConfig, logger *slog.Logger) *OpenAIService {
...
func (s OpenAIService) CompleteRecipe(ctx context.Context, content string) (*Recipe, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("createChatCompletionStream: %w", err)
	}
	...
	defer func() {
		err = reader.Close()
		if err != nil {
			s.logger.WarnContext(ctx, "error closing read stream", slog.Any("error", err))
		}
	}()
